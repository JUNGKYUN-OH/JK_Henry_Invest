...
# 시세 정보
st.subheader(f"{selected.ticker} 시세")
...
st.info(
    f"{icon} **{guide.phase.value}** | "
    f"유효 {float(eff_rnd):.1f}/{max_rnd}회차 | 진행 {cal_rnd}일 | "
    f"평단가 {fmt_usd(avg)} | 보유 {float(shares):.2f}주 | "
    f"잔여 예산 {fmt_usd(guide.remaining_budget)}"
)
...
with right:
    st.subheader("📤 매도 After 지정가 (매일 유지)")
